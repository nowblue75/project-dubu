...
console.log('--- Initiating Perfect Reconstruct Engine v4.0 (Pure 606129b3 Session) ---');
...
function parseViewFileContent(content, targetMap, startLineForce) {
    const lines = content.split('\n');
    let codeStarted = false;
    let currentLineNum = null;
    let count = 0;
    
    // Auto-detect StartLine from "Showing lines X to Y" if present
    let startLine = startLineForce || 1;
    for (const line of lines) {
        const showMatch = line.match(/Showing lines (\d+) to/i);
        if (showMatch) {
            startLine = parseInt(showMatch[1], 10);
            break;
        }
    }
...
logFiles.forEach(logFilePath => {
    ...
    lines.forEach((line, idx) => {
        ...
        try {
            ...
            // --- 1. Replay VIEW_FILE (Rely on internal File Path) ---
            if (log.type === 'VIEW_FILE' && log.status === 'DONE') {
                const logContent = log.content || '';
                
                // Extract file path from logContent
