...
console.log('--- Initiating Perfect Reconstruct Engine v14.0 (Truncation Skip / BOM Polish) ---');
...
function parseViewFileContent(content, targetMap) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        // Strict truncation skip: If the line contains truncation markers, NEVER load or overwrite it!
...
// Fix potential unicode BOM and invisible garbage characters at line 1
if (appArr.length > 0) {
    appArr[0] = appArr[0].replace(/^[^\w\/\*\{\(\[\"\'`\-]+/, '');
}
if (dataArr.length > 0) {
    dataArr[0] = dataArr[0].replace(/^[^\w\/\*\{\(\[\"\'`\-]+/, '');
}
