...
console.log('--- Initiating Perfect Reconstruct Engine v10.0 (Lenient JSON Parser) ---');
...
function lenientParseJSON(str) {
    ...
    // Auto-rescue truncated JSON array by clipping
    let cleanStr = str;
    ...
    if (truncIdx !== -1) {
        ...
        if (lastCloseBrace !== -1) {
            cleanStr = cleanStr.substring(0, lastCloseBrace + 1) + ']';
        } else {
            cleanStr = '[]';
        }
    }
